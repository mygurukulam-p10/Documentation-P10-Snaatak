...
First, we switch to the postgres user using sudo su postgres.Once logged in as the postgres user, we access the PostgreSQL prompt by typing psqland we don't need to put any password for the first time just press enter.This command starts the PostgreSQL interactive terminal, where we can run SQL commands.
